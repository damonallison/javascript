...
test("built-in-map-iterable", () => {
  ...
  m1.forEach((value: number, key: string) => {
  ...
  expect(_.isEqual(pairs, expectedPairs)).toBeTruthy();

  const expectedKeys = new Set<string>(["one", "two", "three"]);
  expect(_.isEqual(keys, expectedKeys)).toBeTruthy();

  const expectedVals = new Set<number>([1, 2, 3]);
  expect(_.isEqual(vals, expectedVals)).toBeTruthy();
});

/**
 * The forEach iterable with sets will *not* guarantee element ordering. Do
 * *not* rely on ordering with sets.
 */
test("built-in-set-iterable", () => {
  const keys = new Set<string>(["one", "two", "three"]);
...
test("generator-manual", () => {
  ...
  expect(
    _.isEqual(["acquiring iterator", "acquired iterator"], activity)
  ).toBeTruthy();
  ...
  expect(
    _.isEqual(
      ["acquiring iterator", "acquired iterator", "before yield"],
      activity
    )
  ).toBeTruthy();
